    ...
    function makeBufferGeometryTexture( geometry , triSize , triVariation){
  
      var data = new Float32Array( SIZE * SIZE  * 4 );
      var color = new Float32Array( SIZE * SIZE  * 4 );
      
      var v1 = new THREE.Vector3();
      var v2 = new THREE.Vector3();
      var v3 = new THREE.Vector3();

      var points = geometry.attributes.position.array;
      var normals = geometry.attributes.normal.array;

      var faceLength = points.length / 9;

      for( var i = 0; i < data.length; i += 4 ){


        var rF =  Math.floor( Math.random() * faceLength );

        v1.set( points[ rF * 9 + 0 ], points[ rF * 9 + 1 ] , points[ rF * 9 + 1 ]);
        v2.set( points[ rF * 9 + 3 ], points[ rF * 9 + 4 ] , points[ rF * 9 + 5 ]);
        v3.set( points[ rF * 9 + 6 ], points[ rF * 9 + 7 ] , points[ rF * 9 + 8 ]);
        ...
        //console.log( f.x );
        data[ i + 3 ] = .1 * areaOfTri( v1 , v2 , v3 ) + .01;


        v1.set( normals[ rF * 9 + 0 ], normals[ rF * 9 + 1 ] , normals[ rF * 9 + 1 ]);
        v2.set( normals[ rF * 9 + 3 ], normals[ rF * 9 + 4 ] , normals[ rF * 9 + 5 ]);
        v3.set( normals[ rF * 9 + 6 ], normals[ rF * 9 + 7 ] , normals[ rF * 9 + 8 ]);
        
        tv1.set(0,0,0);
        tv1.add( v1 );
        tv1.add( v2 );
        tv1.add( v3 );
        //tv1.multiplyScalar( .33333 );
        //tv1.normalize();
        //tv1.multiplyScalar( .5 );
        //tv2.set( .5 ,.5,.5);
        //tv1.add( tv2 );
       // if( i < 1000 ){ console.log( tv1 ); }
      ...
      return { color: makeDataTexture( color ) , position : makeDataTexture( data )}

    }

    function areaOfTri(v1,v2,v3){
    var a = v1.x * ( v2.y - v3.y ) + v2.x * ( v3.y - v1.y ) + v3.x * ( v1.y  - v2.y);
    return a / 2;
    }
